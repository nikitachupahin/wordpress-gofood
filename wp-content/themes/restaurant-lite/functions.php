...
function restaurant_setup() {
	...
	load_theme_textdomain( 'restaurant-lite', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'restaurant-lite' ),
	) );
...
function restaurant_widgets_init() { 	
	...
	register_sidebar( array(
		'name'          => __( 'Blog Sidebar', 'restaurant-lite' ),
		'description'   => __( 'Appears on blog page sidebar', 'restaurant-lite' ),
		'id'            => 'sidebar-1',
		'before_widget' => '',		
...
function restaurant_font_url(){
		...
		*/
		$roboto_condensed = _x('on','roboto_condensed:on or off','restaurant-lite');
		...
		*/
		...
		$playball = _x('on','playball:on or off','restaurant-lite');		
		
		/* Translators: If there has any character that are not supported 
		*  by Scada, translate this to off, do not translate
		*  into your own language.
		*/
		$scada = _x('on','Scada:on or off','restaurant-lite');	
...
function restaurant_credit(){
		return "&copy; 2023 <a href=".home_url('/').">SKT Restaurant & Cafe</a>. All Rights Reserved";		
}
